    ...
    with sqlite3.connect(DATABASE) as conn:
        ...
        for url in urls:
            ...
            for trip in trips:
                ...
                href = trip['href']
                trip_url = "https://r.pl" + href if not href.startswith("http") else href
                ...
                if row:
                    ...
                    if price != stored_price:
                        c.execute('UPDATE trips SET price = ?, last_price = ?, food = ?, persons = ? WHERE trip_hash = ?', (price, stored_price, food, persons_formatted, trip_hash))
                        c.execute('INSERT INTO price_history (trip_hash, price) VALUES (?, ?)', (trip_hash, price))
                else:
                    c.execute('INSERT INTO trips (title, location, days, price, last_price, departure_location, food, persons, trip_hash, trip_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', (title, location, days, price, price, departure_location, food, persons_formatted, trip_hash, trip_url))
                    c.execute('INSERT INTO price_history (trip_hash, price) VALUES (?, ?)', (trip_hash, price))
                ...
                for subscription in subscriptions:
                    ...
                    c.execute('SELECT * FROM subscription_trips WHERE subscription_email = ? AND trip_hash = ? AND trip_price = ?', (email, trip_hash, price))
                    row = c.fetchone()

                    if price <= desired_price and not row:
                        ...
                        c.execute('INSERT INTO subscription_trips (subscription_email, trip_hash, trip_price) VALUES (?, ?, ?)', (email, trip_hash, price))
